        ...
        while (True):
            if queue.qsize()<100:
                ...
                for k in range(batchsize):
                    r = np.random.rand(1)
                    if (lstep == LearningStep.TRAINING):
                        if (r<0.33):                        
                            ...
                            im[k], reg[k] = imageGetter.getImage(DB, slidelist_train, IMAGESIZE_X, IMAGESIZE_Y, learningStep=lstep,ds=16,basepath=basepath,hardExamplePick=True)
                    elif (lstep == LearningStep.VALIDATION):
                        im[k], reg[k] = imageGetter.getImage(DB, slidelist_train, IMAGESIZE_X, IMAGESIZE_Y, learningStep=lstep,ds=16,basepath=basepath, allAreaPick=True)
                    else: # test
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            for epoch in np.arange(max_epoch-19, max_epoch+1):
                ...
                RMSE=np.sqrt(np.mean(np.square(np.array(signed_errors))))
                results['epoch'].append( epoch )
                results['RMSE'].append ( RMSE)

                MAE=np.mean(np.abs(np.array(signed_errors)))
                results['MAE'].append ( MAE)

                print('epoch: %3d RMSE: %f  MAE: %f ' % (epoch,RMSE, MAE))
            ...
            plt.figure()
            plt.plot(results['epoch'], results['RMSE'], label='RMSE')
            plt.grid()
            plt.legend()
            plt.savefig(tag+'RMSE_dirreg_xv%s.pdf' % sys.argv[1])

            plt.figure()
            plt.plot(results['epoch'], results['MAE'], label='MAE')
            plt.legend()
            plt.grid()
            plt.savefig(tag+'MAE_dirreg_xv%s.pdf' % sys.argv[1])

            np.savez(tag+'_validation_%s' % sys.argv[1], RMSE=RMSE, MAE=MAE)
